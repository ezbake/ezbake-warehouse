...
import ezbake.warehouse.WarehouseConstants;
import ezbakehelpers.accumulo.AccumuloHelper;

public class WarehouseExport extends AbstractWarehouseAction {
	...
	public static void main(String[] args) throws TException, IOException, EzConfigurationLoaderException {
		...
		WarehouseExport exporter = new WarehouseExport();
        CmdLineParser parser = new CmdLineParser(exporter);

        try {
            parser.parseArgument(args);
            exporter.process();
            System.out.println("WarehouseExport started");
    ...
    public void process() throws TException, IOException, EzConfigurationLoaderException {
        ...
        if (!end.equals("null")) {
            finishTime = ToolHelper.thriftConvert(end);
        }
        
        ByteBuffer data = export.exportTestData(name, startTime, finishTime, new Visibility().setFormatVisibility("U"), token);
	...
	public ByteBuffer exportTestData(String urn, DateTime start, DateTime finish, 
	        Visibility visibility, EzSecurityToken security) throws TException {
        ...
        try {
            ...
            RegExFilter.setRegexs(iterator,
                    WarehouseUtils.getPatternForURI(urn).toString(), null, null, null, false);
    ...
    private static BatchScanner createScanner(String auths) throws TException {
        ...
        try {
            try {
               Properties config = new DirectoryConfigurationLoader().loadConfiguration();
                connector = new AccumuloHelper(config).getConnector(true);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            return connector.createBatchScanner(WarehouseConstants.TABLE_NAME,
                    WarehouseUtils.getAuthsFromString(auths), WarehouseConstants.QUERY_THREADS);
