...
import ezbake.warehouse.ExportFile;
import ezbake.warehouse.VersionControl;
import ezbakehelpers.accumulo.AccumuloHelper;

public class WarehouseImport extends AbstractWarehouseAction {
    ...
    public static void main(String[] args) throws IOException, TException, EzConfigurationLoaderException {
        ...
        WarehouseImport importer = new WarehouseImport();
        CmdLineParser parser = new CmdLineParser(importer);

        try {
            parser.parseArgument(args);
            importer.process();
            System.out.println("WarehouseImport started");
        ...
        try {
            ...
            for (String uri : uriList) {
                ...
                if (parsedMap.containsKey(uri)) {
                    mutation.put(new Text(WarehouseUtils.getUriPrefixFromUri(uri)), new Text(ExportImportHelper.DataType.PARSED.toString()),
                ...
                if (rawMap.containsKey(uri)) {
                    mutation.put(new Text(WarehouseUtils.getUriPrefixFromUri(uri)), new Text(ExportImportHelper.DataType.RAW.toString()),
                            visibilityForUpdate, timestamp,
                            new Value(ThriftUtils.serialize(
    ...
    private BatchWriter createWriter() throws TException {
        try {
            ...
            if (!connector.tableOperations().exists(WarehouseConstants.TABLE_NAME)) {
                throw new TException(WarehouseConstants.TABLE_NAME + "does not exist");
            }

            BatchWriter writer = connector.createBatchWriter(WarehouseConstants.TABLE_NAME, maxMemory, latency, threads);
            return writer;
        } catch (IOException e) {
            throw new TException(e);
