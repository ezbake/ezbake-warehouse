...
public class WarehouseServer {
	private static AccumuloWarehouse handler;

	@SuppressWarnings("rawtypes")
   private static WarehouseService.Processor processor;
   ...
   public static void main(String[] args) {
		try {
			...
			config.put(EzbakeSecurityClient.USE_MOCK_KEY, "true");
			config.put(EzBakePropertyConstants.EZBAKE_SECURITY_ID, "test");
			handler = new AccumuloWarehouse();
			handler.setConfigurationProperties(config);
			processor = (WarehouseService.Processor) handler.getThriftProcessor();
