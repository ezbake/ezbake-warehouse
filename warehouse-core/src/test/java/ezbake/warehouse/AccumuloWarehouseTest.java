...
public class AccumuloWarehouseTest {
	...
	private AccumuloWarehouse warehouse;
	private FileSystem hdfs;
	
	@Before
	public void beforeEach() throws Exception {
		warehouse = new AccumuloWarehouse();
		warehouse.setConfigurationProperties(config);
        warehouse.getThriftProcessor();
		calendar.set(Calendar.MILLISECOND,0);
	}

	@After
	public void afterEach() throws Exception {
		warehouse.resetTable();
	}
	
	public AccumuloWarehouseTest() throws Exception {
	...
	@Test
	public void replayTest() throws TException, InterruptedException, EntryNotInWarehouseException {
		...
		warehouse.insert(repository, visibility, token);
        Thread.sleep(2000);
		...
		warehouse.insert(repository, visibility, token);
        Thread.sleep(2000);
		List<DatedURI> result = warehouse.replay("this://testfeed", false, null, null, null, token);
        ...
        BinaryReplay version = warehouse.getParsed(result.get(0).getUri(), TimeUtil.convertFromThriftDateTime(result.get(0).getTimestamp()), token);
		...
		result = warehouse.replay("this://testfeed", false, null, endTime, null, token);
		...
		DateTime startTime = TimeUtil.convertToThriftDateTime(
                TimeUtil.convertFromThriftDateTime(result.get(1).getTimestamp()) - 1000);
		result = warehouse.replay("this://testfeed", false, startTime, null, null, token);
		assertEquals(2,result.size());
		assertTrue(compare(result.get(0).getTimestamp(),result.get(1).getTimestamp()) < 0);
		...
		startTime = TimeUtil.convertToThriftDateTime(
                        TimeUtil.convertFromThriftDateTime(result.get(0).getTimestamp()) - 1000);
		result = warehouse.replay("this://testfeed", false, startTime, endTime, null, token);
		assertEquals(1,result.size());
		assertEquals(repositories.get(1).getUri(), result.get(0).getUri());
		
		try {
	        result = warehouse.replay(null, false, null, null, null, token);
        ...
        warehouse.insert(repository, visibility, token);
        Thread.sleep(1000);
        List<DatedURI> latest = warehouse.replay("this://testfeed", false, null, null, null, token);
        assertEquals(4, latest.size());
        latest = warehouse.replay("this://testfeed", true, null, null, null, token);
        assertEquals(3, latest.size());
	}

    @Test
    public void replayTest_BulkInsert() throws TException, InterruptedException, EntryNotInWarehouseException {
        ...
        long timestamp = warehouse.put(request, token).getTimestamp();
        ...
        List<DatedURI> replayResults = warehouse.replay(prefix, true, start, end, null, token);
    ...
    @Test
    public void replayTest_ReplayLatestWithNewerReport() throws TException, InterruptedException, EntryNotInWarehouseException {
        int numOfEntries = 10;
        PutRequest request = new PutRequest();
        String prefix = "test://replay";
        ...
        long startTs = warehouse.put(request, token).getTimestamp();

        // Insert uri suffix 0 again
        UpdateEntry update = new UpdateEntry();
        update.setParsedData("0".getBytes());
        update.setUri(prefix + "/0");
        long endTs = warehouse.updateEntry(update, visibility, token).getTimestamp();
    ...
    @Test
    public void replayTest_WithNewerReportDontReplayOnlyLatest() throws TException, InterruptedException, EntryNotInWarehouseException {
        int numOfEntries = 10;
        PutRequest request = new PutRequest();
        String prefix = "test://replay";
        ...
        // Run replay and validate that we get all the URIs we expect
        List<DatedURI> replayResults = warehouse.replay(prefix, false, start, end, null, token);
    ...
    public void replayTest_RealisticURI() throws TException, InterruptedException {
        List<Repository> repositories = Lists.newLinkedList();
        Repository repository = new Repository();
        repository.setUri("SOCIAL://test/social.twitter.com:otherstuff,WOOOO:./");
        repository.setRawData(UUID.randomUUID().toString().getBytes());
        repository.setParsedData(UUID.randomUUID().toString().getBytes());
        repositories.add(repository);
        warehouse.insert(repository, visibility, token);
        Thread.sleep(50);
        repository = new Repository();
        repository.setUri("SOCIAL://test/social.twitter.com:ANOTHERone,WOOOO:./");
        repository.setRawData(UUID.randomUUID().toString().getBytes());
        repository.setParsedData(UUID.randomUUID().toString().getBytes());
        repositories.add(repository);
        warehouse.insert(repository, visibility, token);
        ...
        List<DatedURI> result = warehouse.replay("SOCIAL://test", false, null, null, null, token);
        assertEquals(2,result.size());
        assertTrue(compare(result.get(0).getTimestamp(),result.get(1).getTimestamp()) < 0);
        assertEquals(repositories.get(0).getUri(),result.get(0).getUri());
        assertEquals(repositories.get(1).getUri(),result.get(1).getUri());
    ...
    public void replayTest_ReplayRawData() throws TException, InterruptedException {
        ...
        UpdateEntry update = new UpdateEntry();
        update.setUri("SOCIAL://test/social.twitter.com:otherstuff,WOOOO:./");
        ...
        warehouse.updateEntry(update, visibility, token);
        Thread.sleep(50);

        // Only provide a RAW object for this entry
        update = new UpdateEntry();
        update.setUri("SOCIAL://test/social.twitter.com:ANOTHERone,WOOOO:./");
        update.setRawData(UUID.randomUUID().toString().getBytes());
        repositories.add(update);
        warehouse.updateEntry(update, visibility, token);
        ...
        result = warehouse.replay("SOCIAL://test", false, null, null, GetDataType.RAW, token);
        assertEquals(2, result.size());
        assertTrue(compare(result.get(0).getTimestamp(),result.get(1).getTimestamp()) < 0);
        assertEquals(repositories.get(0).getUri(),result.get(0).getUri());
        assertEquals(repositories.get(1).getUri(),result.get(1).getUri());
    ...
    public void replayTest_InvalidDataType() throws TException, InterruptedException {
        ...
        warehouse.replay("SOCIAL://test", false, null, null, GetDataType.VIEW, token);
    }

    @Test
    public void testUpdate_ParsedOnly() throws TException, InterruptedException, EntryNotInWarehouseException {
        ...
        BinaryReplay result = warehouse.getLatestParsed(update.getUri(), token);
        assertArrayEquals("Contents are equal", update.getParsedData(), result.getPacket());
    }

    @Test
    public void testUpdate_RawThenParsed() throws TException, InterruptedException, EntryNotInWarehouseException {
        List<UpdateEntry> updates = Lists.newLinkedList();
        String uri = "this://testfeed/" + UUID.randomUUID().toString();
        ...
        BinaryReplay result = warehouse.getLatestParsed(uri, token);
        ...
        result = warehouse.getLatestRaw(uri, token);
        assertArrayEquals("Contents of raw are equal", updates.get(1).getRawData(), result.getPacket());

        result = warehouse.getLatestParsed(uri, token);
        assertArrayEquals("Contents of parsed are equal", updates.get(0).getParsedData(), result.getPacket());


        // Verify that replay returns 1 URI since we only return parsed objects
        List<DatedURI> replayResults = warehouse.replay("this://testfeed", false, startTime, endTime, null, token);
        assertEquals("Should only have 1 result", 1, replayResults.size());
    }

    @Test
    public void testUpdate_UpdateTypeForEntry() throws TException, InterruptedException, EntryNotInWarehouseException {
        List<UpdateEntry> updates = Lists.newLinkedList();
        ...
        List<DatedURI> replayResults = warehouse.replay(uri, false, startTime, endTime, null, token);
        assertEquals("Should only have 2 results", 2, replayResults.size());
    }

    @Test
    public void testUpdate_UpdateParsedNewVisibility{
        List<UpdateEntry> updates = Lists.newLinkedList();
        ...
        BinaryReplay result = warehouse.getLatestParsed(uri, token);
        assertArrayEquals("Contents of parsed are equal", updates.get(0).getParsedData(), result.getPacket());
        List<DatedURI> replayResults = warehouse.replay(prefix, false, null, null, null, token);
        ...
        update.setParsedData(UUID.randomUUID().toString().getBytes());
        updates.add(update);
        warehouse.updateEntry(update, new Visibility().setFormalVisibility("U&USA"), token);
        Thread.sleep(100);
        DateTime endTime = TimeUtil.convertToThriftDateTime(System.currentTimeMillis() + 1000);
        ...
        // Verify that replay returns 2 URIs since we only retrieve parsed objects
        replayResults = warehouse.replay(prefix, false, startTime, endTime, null, token);
        assertEquals("Should only have 2 results", 2, replayResults.size());
        assertEquals("Old visibility", "U", replayResults.get(0).getVisibility().getFormalVisibility());
    }

    @Test
    public void testUpdate_MultipleTypes() throws TException, InterruptedException, EntryNotInWarehouseException {
        List<UpdateEntry> updates = Lists.newLinkedList();
        String uri = "this://" + UUID.randomUUID().toString();
        DateTime startTime = TimeUtil.convertToThriftDateTime(System.currentTimeMillis());
        UpdateEntry update = new UpdateEntry();
        ...
        BinaryReplay result = warehouse.getLatestParsed(uri, token);
        assertArrayEquals("Contents of parsed are equal", updates.get(0).getParsedData(), result.getPacket());
        List<DatedURI> replayResults = warehouse.replay(uri, false, null, null, null, token);
        assertEquals("Correct visibility", "U", replayResults.get(0).getVisibility().getFormalVisibility());
        ...
        update.setRawData(UUID.randomUUID().toString().getBytes());
        updates.add(update);
        warehouse.updateEntry(update, new Visibility().setFormalVisibility("U&FOUO"), token);
        Thread.sleep(1000);

        result = warehouse.getLatestParsed(uri, token);
        assertArrayEquals("Contents of parsed are equal", updates.get(0).getParsedData(), result.getPacket());
        replayResults = warehouse.replay(uri, false, null, null, null, token);
        ...
        update.setParsedData(UUID.randomUUID().toString().getBytes());
        updates.add(update);
        warehouse.updateEntry(update, new Visibility().setFormalVisibility("S"), token);
        Thread.sleep(50);
        ...
        // Verify that replay returns 2 URIs, since we only retrieve parsed objects
        replayResults = warehouse.replay(uri, false, startTime, endTime, null, token);
        assertEquals("Should only have 2 results", 2, replayResults.size());
        assertEquals("Old visibility", "U", replayResults.get(0).getVisibility().getFormalVisibility());
    }

    @Test(expected = EntryNotInWarehouseException.class)
    public void entryNotInWarehouse() throws TException, EntryNotInWarehouseException {
        warehouse.getLatestParsed("fake", token);
    }
	
	@Test
	public void versionsTest() throws TException, InterruptedException, EntryNotInWarehouseException {
		...
		timestamps[0] = warehouse.insert(repository, visibility, token).getTimestamp();
        Thread.sleep(50);
		repository = new Repository();
		repository.setUri(uri);
		...
		timestamps[1] = warehouse.insert(repository, visibility, token).getTimestamp();
        ...
        timestamps[2] = warehouse.insert(repository, visibility, token).getTimestamp();
		List<Long> versions = warehouse.getVersions(uri, token);
		assertEquals(3,versions.size());
		assertTrue(versions.get(0) > versions.get(1)); // most recent one comes first
        assertTrue(versions.get(1) > versions.get(2));
        Thread.sleep(500);
		
		BinaryReplay br1 = warehouse.getParsed(uri, timestamps[0], token);
		BinaryReplay br2 = warehouse.getLatestParsed(uri, token);
...
//	@Test
	public void importTest() throws TException, IOException, EntryNotInWarehouseException {
		...
		String filename = "datawarehousetestfile.txt";
        ...
        try {
            ...
            os.flush();
            os.close();
            warehouse.importFromHadoop(filename, visibility, token);
            assertEquals(raw,new String(warehouse.getLatestRaw(uri, token).getPacket()));
            assertEquals(parsed,new String(warehouse.getLatestParsed(uri,token).getPacket()));
        } finally {
	...
	@Test
	public void viewsTest() throws TException, InterruptedException, EntryNotInWarehouseException {
		ViewId id = new ViewId("view_test_uri","testcases","myview");
		long ts1 = warehouse.insertView(ByteBuffer.wrap("show me this".getBytes()), id, visibility, token).getTimestamp();
		Thread.sleep(1000);
		long ts2 = warehouse.insertView(ByteBuffer.wrap("show me that instead".getBytes()), id, visibility, token).getTimestamp();
		assertTrue(ts1 < ts2);
		BinaryReplay val1 = warehouse.getView(id, ts1, token);
		BinaryReplay val2 = warehouse.getLatestView(id, token);
	...
	public void smugglingTest() throws TException, InterruptedException {
		...
		data.setRawData(raw.getBytes());
		data.setParsedData(parsed.getBytes());
		warehouse.insert(data, visibility, token);
		Thread.sleep(1000);
		int count = warehouse.replayCount(uri, null, null, null, token);
	...
	public void insertPurgeStatus_NullState() {
	   ...
	   try {
         warehouse.insertPurgeStatus(null);
   ...
   //@Test
	public void insertPurgeStatus_Basic() throws TException {
      ...
      this.warehouse.insertPurgeStatus(state);
      
      PurgeState fetchedState = this.warehouse.purgeStatus(token, purgeId);
   ...
   //@Test
	public void beginPurge_NullIdsToPurge() throws PurgeException, EzSecurityTokenException, TException {
      ...
      PurgeState state = warehouse.beginPurge("purgeCallbackService", purgeId, idsToPurge, token);
   ...
   public void purgeStatus_PurgeIdNotFound() throws TException {
    ...
    public void remove_SingleUri() throws Exception {
        ...
        long timestamp = warehouse.insert(data, visibility, token).getTimestamp();
        Thread.sleep(1000);
        
        // Verify that inserted item is in the warehouse.
        try {
            warehouse.getParsed(uri, timestamp, token);
        } catch (EntryNotInWarehouseException e) {
            fail("Received an EntryNotInWarehouseException; should not have seen this exception");
        }
        int count = warehouse.replayCount(uri, null, null, null, token);
        assertEquals(1, count);
        ...
        uris.add(uri);
        warehouse.remove(uris, token);
        
        // Verify that deleted item is not in the warehouse.
        try {
           warehouse.getLatestParsed(uri, token);
           fail("When getLastestParsed is called using the uri that was deleted then an EntryNotInWarehouseException should be thrown.");
        } catch (EntryNotInWarehouseException e) {
        }
        count = warehouse.replayCount(uri, null, null, null, token);
    ...
    public void remove_MultipleUris() throws Exception {
       ...
       long timestamp1 = warehouse.insert(data, visibility, token).getTimestamp();

       data.setUri(uri2);
       data.setRawData("222222222222".getBytes());
       data.setParsedData("two".getBytes());
       long timestamp2 = warehouse.insert(data, visibility, token).getTimestamp();

       data.setUri(uri3);
       data.setRawData("33333333333".getBytes());
       data.setParsedData("three".getBytes());
       long timestamp3 = warehouse.insert(data, visibility, token).getTimestamp();
       Thread.sleep(1000);

       // Verify that inserted item is in the warehouse.
       try {
          warehouse.getParsed(uri1, timestamp1, token);
       } catch (EntryNotInWarehouseException e) {
          fail("Received an EntryNotInWarehouseException for uri1; should not have seen this exception");
       }
       try {
          warehouse.getParsed(uri2, timestamp2, token);
       } catch (EntryNotInWarehouseException e) {
          fail("Received an EntryNotInWarehouseException for uri2; should not have seen this exception");
       }
       try {
          warehouse.getParsed(uri3, timestamp3, token);
       } catch (EntryNotInWarehouseException e) {
          fail("Received an EntryNotInWarehouseException for uri1; should not have seen this exception");
       }

       warehouse.remove(Arrays.asList(uri1, uri2, uri3), token);

       // Verify that deleted items are not in the warehouse.
       List<Long> versions = warehouse.getVersions(uri1, token);
       assertEquals("Uri1 should have been deleted.", 0, versions.size());

       versions = warehouse.getVersions(uri2, token);
       assertEquals("Uri2 should have been deleted.", 0, versions.size());

       versions = warehouse.getVersions(uri3, token);
    ...
    public void remove_WithUpdates() throws Exception {
       ...
       List<Long> versions = warehouse.getVersions(data.getUri(), token);
       assertEquals("Verify version count before remove.", 3, versions.size());

       warehouse.remove(Arrays.asList(data.getUri()), token);
       
       Thread.sleep(200);
       versions = warehouse.getVersions(data.getUri(), token);
    ...
    public void cancelPurge_WithoutToken() throws EzSecurityTokenException, TException {
       ...
       PurgeState status = warehouse.cancelPurge(null,  new Date().getTime());
    ...
    public void cancelPurge_BasicCase() throws EzSecurityTokenException, TException {
       ...
       PurgeState status = warehouse.cancelPurge(token,  new Date().getTime());
       assertEquals(status, CancelStatus.CANNOT_CANCEL);
    }
    ...
    public void testGet() throws TException, InterruptedException {
        ...
        List<ezbake.warehouse.BinaryReplay> getResults = null;
        
        try {
            getResults = warehouse.get(getRequest, token);
        ...
        long ts1 = warehouse.insertView(ByteBuffer.wrap("some gobbledygook".getBytes()), id, visibility, token).getTimestamp();
        Thread.sleep(100);
        
        id = new ViewId("view_test_uri_2", "testcases", "myview2");
        long ts2 = warehouse.insertView(ByteBuffer.wrap("some more gobbledygook".getBytes()), id, visibility, token).getTimestamp();
        Thread.sleep(100);
        ...
        repositories.add(repository);
        long ts = warehouse.insert(repository, visibility, token).getTimestamp();
        Thread.sleep(50);
        ...
        getRequest.setLatestVersion(false);
        try {
            warehouse.get(getRequest, token);
    ...
    public void testPut() throws TException, InterruptedException {
        ...
        // initial insert
        long ts1 = warehouse.put(putReq, token).getTimestamp();
        Thread.sleep(100);
        DateTime endTime = TimeUtil.convertToThriftDateTime(System.currentTimeMillis() + 1000);
        
        // Verify that replay returns one record per uri
        List<DatedURI> replayResults = warehouse.replay(uri1, false, startTime, endTime, null, token);
        assertEquals("Should have 1 result", 1, replayResults.size());
        replayResults = warehouse.replay(uri2, false, startTime, endTime, null, token);
        assertEquals("Should have 1 result", 1, replayResults.size());
        replayResults = warehouse.replay(uri3, false, startTime, endTime, null, token);
        assertEquals("Should have 1 result", 1, replayResults.size());
        ...
        // subsequent update
        long ts2 = warehouse.put(putReq, token).getTimestamp();
        Thread.sleep(100);
        ...
        replayResults = warehouse.replay(uri1, false, startTime, endTime, null, token);
        ...
        try {
            BinaryReplay result = warehouse.getLatestParsed(updateEntries.get(2).getEntry().getUri(), token);
            assertEquals("Contents are equal", "part of a bulk update", new String(result.getPacket()));
        } catch (EntryNotInWarehouseException ex) {
        ...
        long ts3 = warehouse.put(putReq, token).getTimestamp();
        Thread.sleep(100);
        endTime = TimeUtil.convertToThriftDateTime(System.currentTimeMillis() + 1000);
    ...
    public void testGetEntryDetails() throws TException, InterruptedException {
        ...
        repository.setRawData(UUID.randomUUID().toString().getBytes());
        repository.setParsedData(UUID.randomUUID().toString().getBytes());
        warehouse.insert(repository, new Visibility().setFormalVisibility("U"), token);
        Thread.sleep(50);
        ...
        EntryDetail detail = warehouse.getEntryDetails(uri, token);
        ...
        assertEquals("warehouse-test", detail.getVersions().get(0).getSecurityId());
        assertEquals("warehouse-test", detail.getVersions().get(1).getSecurityId());
    ...
    public void testPut_ExternalCommunityMarkings() throws TException, InterruptedException {
        ...
        IngestStatus status = warehouse.put(putReq, token);
        ...
        try {
            warehouse.getLatestParsed(uri1, token);
            assertTrue("An exception should have been thrown here", false);
        } catch (EntryNotInWarehouseException e) {
            assertTrue("Was not able to see the entry with communities", true);
        }

        EzSecurityToken tokenWithCommunities = TestUtils.createTestToken(String.valueOf("warehouse"),
                                                Sets.newHashSet("U", "S", "C", "FOUO", "USA"),
                                                String.valueOf("some_id"));
        tokenWithCommunities.getAuthorizations().setExternalCommunityAuthorizations(Sets.newHashSet("TEST"));

        BinaryReplay result = warehouse.getLatestParsed(uri1, tokenWithCommunities);
        assertNotNull("Got the object back from the warehouse", result);
    ...
    public void testPermissions() throws TException, InterruptedException {
        ...
        long ts = 0;
        IngestStatus status = warehouse.insert(repository, 
        ...
        status = warehouse.insert(repository, 
            new Visibility().setFormalVisibility("S").setAdvancedMarkings(
        ...
        try {
            warehouse.getParsed(uri, ts, fouoToken);
        ...
        try {
            warehouse.getParsed(uri, ts, secretToken);
        } catch (EntryNotInWarehouseException e) {
        ...
        update.setUri(uri);
        status = warehouse.updateEntry(update, new Visibility().setFormalVisibility("S"), fouoToken);
        assertTrue("An exception should have been thrown here", status.getStatus() == IngestStatusEnum.FAIL);

        status = warehouse.updateEntry(update, new Visibility().setFormalVisibility("S"), secretToken);
        ...
        status = warehouse.insert(repository2, visWithPov, tokenWithPoa);
        ...
        try {
            warehouse.getLatestParsed(uri2, tsToken);
        ...
        try {
            warehouse.getLatestParsed(uri2, tokenWithPoa);
        } catch (EntryNotInWarehouseException e) {
        ...
        status = warehouse.updateEntry(someupdate, visWithPov, tokenWithPoa);
        ...
        status = warehouse.insert(repository3, visWithPov, tokenWithPoa);
